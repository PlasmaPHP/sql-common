...
class TimestampWithTimeZoneType implements ColumnTypeInterface {
    /**
     * Get the SQL string.
    ...
    static function getSQL(?\Plasma\SQL\GrammarInterface $grammar, \Plasma\SQL\Blueprints\ColumnBlueprint $column): string {
        if(!($grammar instanceof \Plasma\SQL\Grammar\PostgreSQL)) {
            throw new \LogicException('TIMESTAMP WITH TIME ZONE does only exist in PostgreSQL');
        }
        
        return 'TIMESTAMP'.($column->getLength() >= 0 ? '('.$column->getLength().')' : '').' WITH TIME ZONE';
    }
}
