...
class VarbitType implements ColumnTypeInterface {
    /**
     * Get the SQL string.
    ...
    static function getSQL(?\Plasma\SQL\GrammarInterface $grammar, \Plasma\SQL\Blueprints\ColumnBlueprint $column): string {
        if($column->getLength() <= 0) {
            throw new \InvalidArgumentException('VARBIT does not accept negative, null or zero length');
        }
        
        return 'VARBIT('.$column->getLength().')';
    }
}
