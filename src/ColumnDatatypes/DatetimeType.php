...
class DatetimeType implements ColumnTypeInterface {
    /**
     * Get the SQL string.
    ...
    static function getSQL(?\Plasma\SQL\GrammarInterface $grammar, \Plasma\SQL\Blueprints\ColumnBlueprint $column): string {
        if(!($grammar instanceof \Plasma\SQL\Grammar\MySQL)) {
            throw new \LogicException('DATETIME only exists in MySQL');
        }
        
        return 'DATETIME'.($column->getLength() >= 0 ? '('.$column->getLength().')' : '');
    }
}
